---
title: 'Como funciona o Garbage Collection no Node.js'
date: '2026-09-19'
lastmod: '2026-09-22'
draft: false
tags: ['node.js','v8', 'garbage-collection']
summary: 'Entendendo como funciona o Garbage Collection no Node.js.'
---
# Introdução

Esse artigo um estudo sobre o Garbage Collection no V8, e consequentemente no Node.js, ele não tem a intenção de apresentar algo prático com aplicações diretas no dia a dia, apesar de apresentar código e exemplos a intenção aqui é aprofundar o conhecimento sobre Node.js e como ele funciona.


De forma geral, todo Garbage collector funciona dessa forma:
...
A definição de objetos vivos e mortos depende do "alcance": em JavaScript, um objeto está vivo se pode ser alcançado a partir da raiz. Um objeto morto não pode ser alcançado.
...
O V8 divide a heap em duas áreas chamadas generations (gerações), sendo elas a *Young generation* e a *Old generation*. A *Young generation* tem mais duas divisões, *"Nursery"* e *"Intermediate"*. São nomes conceituais, na seção sobre o Minor aprenseto os nomes corretos. 
...
Esse GC é responsável por mover objetos entre as duas divisões da *Young generation*, dividida em dois *semi-spaces*, **From-Space** e **To-Space**. Como a *Young generation* é menor, ~16 MB, o Minor GC é executado com mais frequência.
...
# Sinas de vazamento de memória (o que procurar nos logs?)

### 1. O uso de memória aumenta depois do Major GC. 

O esperado é que após o Major GC a Old generation fique menor ou se mantenha estável, isso significa que o Major GC está liberando a memória enquantos objetos de Young são introduzidos.

Retorno esperado - Major GC funcionando, memória se mantém ~30MB.
...
### 2. Minor parece não funcionar.

Quando muitos objetos vão da Young para Old pode ser o indício de um problema dado que poderá sobrecarregar a Old. Nos logs você verá que a memória usada antes e depois do Minor GC (Scavenge) não diminui, mesmo após sucessivas execuções do Minor.
...
### 3. Major GC sendo executado com maior frequência.

Quando a Old fica sem espaço o Major GC precisa limpa-la. Como já sabemos o esperado é que boa parte dos objetos não sejam promividos da Young para a Old, e se a Old fica constantemente cheia você irá notar o Major GC sendo executado com mais frequência e num menor espaço de tempo entre as execuções.

Nos logs você verá algo assim:
...
Após entender como funciona o GC no Node, podemos ler o trace do GC e identificar se há vazamento de memória, porém não conseguimos identificar onde ele ocorre, para isso devemos usar outras ferramentas. Em breve escrevei um artigo sobre como identificar esses vazamentos.
